export const LAYERS = {
    ...
    principles: [
      {
        ...
        rules: [
          ...
          "Icon-only controls with circular or square hit areas declare explicit centering with grid or flex alignment. Do not rely on browser button line-height or default icon placement.",
          "Markers, dots, and handles on a visible track or rail align to the track's visual centre.",
          "A responsive state exposes one clear visible control for one action. Do not show duplicate close, expand, save, or navigation controls that compete for the same action.",
          ...
          "No magic numbers: sizes, spacing, and radius come from the design scale or tokens, not one-off literals. Check CSS, inline styles (style={{ padding: '13px' }}), and JS style objects.",
      ...
      },
      {
        ...
        rules: [
          ...
          "No hardcoded color values: hex (#3b82f6), rgb(), hsl(), or named colors in component code. Extract to a design token.",
          ...
          "Brand marks retain legible contrast across light and dark surfaces. Use an approved inverse asset or colour treatment when a standard mark fails.",
      ...
      },
      {
        ...
        rules: [
          ...
          "Use clear punctuation appropriate to the context; do not use em dashes or en dashes in product copy.",
      ...
      },
      {
        ...
        rules: [
          ...
          "Form control rhythm is explicit: label-to-control spacing and control-group spacing are consistent. Do not let fieldset or legend browser defaults collapse or erase the gap after a preceding control.",
          "Heading-and-action rows align by visual centre or baseline even when one control has a larger touch target than its neighbouring text.",
          "Full-bleed surfaces span the intended viewport edge to edge. Do not accidentally constrain a hero or media surface inside a content container.",
          "Logical properties (margin-inline-start, padding-inline-end) instead of left/right.",
          "No fixed widths or heights on text containers; let text define its own size. Also avoid fixed dimensions on buttons, inputs, and cards in CSS or inline styles.",
          "Consistent, adequate whitespace; no cramped or cluttered layouts."
        ]
      },
      {
        ...
        rules: [
          ...
          "Auto-rotating content stays readable: use a calm interval, pause on hover and focus, provide manual controls, and stop or simplify it for reduced-motion preferences."
        ]
      },
      {
        id: "C6",
        ...
        rules: [
          ...
          "No inline style overrides that bypass the design system. Check both JSX style={{ }} objects and imperative style assignments.",
          "State lives at the right level; no values that change every frame in component state.",
          "Related screens in the same product share the same type scale, spacing scale, action treatment, and card geometry unless a deliberate difference is evident."
        ]
      },
      {
        id: "C7",
        ...
        rules: [
          ...
          "Headings and subheadings say what the section helps the reader do; avoid vague labels and repeated summaries.",
          "Descriptions and helper text are specific, concise, and useful without filler such as 'seamless', 'powerful', 'robust', or 'take your experience to the next level'.",
          "Avoid common AI-writing signals: generic introductions, padded conclusions, repetitive sentence structure, excessive bullet symmetry, abstract claims without evidence, and unnecessary restatement.",
          "Treat patterns commonly associated with AI writing as copy-quality risks, not proof of a text's origin. Never claim that text was AI-generated.",
          "Tie claims of importance, impact, or transformation to an observable product-specific reason. Remove generic claims such as 'pivotal', 'vital', or 'a testament' when they do not add evidence.",
          "Do not use vague authority as a substitute for evidence, such as 'experts say', 'industry reports', or 'many users'. Name the source when it matters, or state the concrete fact instead.",
          "Use formulaic contrasts such as 'not just X, but Y' only when the contrast adds a specific, necessary distinction. Avoid rhetorical contrast used only to make ordinary claims sound larger.",
          "Do not add conclusion, future outlook, challenges, or summary copy that merely restates the section or speculates without a useful next action.",
          "Do not add eyebrow or overline labels unless they provide context that the heading cannot provide.",
          "Use a human, direct voice and preserve the product's meaning; propose replacement copy when wording is the issue.",
          "Do not use em dashes or en dashes in product copy; use commas, parentheses, colons, or separate sentences instead."
        ]
      },
      {
        id: "C8",
        name: "Rendered visual QA",
        rules: [
          "When rendered screenshots are supplied, inspect the actual result at each viewport, not just the source code.",
          "Check visible icon centring inside controls, vertical and horizontal alignment between adjacent controls, spacing between form groups, and duplicate affordances in open or responsive states.",
          "Check that primary media and heroes have the intended crop, width, and readable text contrast at the supplied viewport.",
          "Compare related screens supplied in the same review for visual-system drift in hierarchy, typography, spacing, actions, and card treatment.",
          "When no rendered visual evidence is supplied, do not report screenshot-only claims as facts. Report a code-level risk only when the source makes it unambiguous."
        ]
      }
    ]
  },
  ...
  accessibility: {
    ...
    accessibility: {
      ...
      name: "Accessibility",
      rules: [
        "Text contrast at least 4.5:1 for body text, 3:1 for large text. Flag hardcoded colors (hex, rgb, named) used for text without verifying contrast against their actual background. Check that dark-mode tokens exist for every light-mode color used on text.",
